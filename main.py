...
webview.iconChanged.connect(lambda icon,webpage_icon_change=webview:tab.setTabIcon(tab.indexOf(webpage_icon_change),icon))
def close_tab(index):
    current_webpage=tab.widget(index)
...
title=webview.title()
webview.titleChanged.connect(lambda:tab.setTabText(tab.currentIndex(),webview.title()))
tab.addTab(webview,title)
